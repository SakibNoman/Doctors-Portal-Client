...
const BookAppointment = ({ date }) => {
    return (
        <section>
            <h2 className="text-center text-fresh mb-5" >Available Appointments on {date.toDateString()}</h2>
            <div className="row">
                {
                    bookingData.map(each => <BookingCard key={each._id} date={date} booking={each} ></BookingCard>)
                }
            </div>
